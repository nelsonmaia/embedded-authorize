/**
 * vite-plugin-tenant-proxy.js — live-tenant calls, the way Postman makes them.
 *
 ...
 * does the same thing from the Vite dev server.
 *
 ...
 * down.
 *
 * `apply: 'serve'` means this cannot exist in a production build — there is no way to accidentally
 * ship an open proxy.
 *
 * Contract:  POST /__tenant   { domain, path, method, body, contentType? }
 *            → { ok: true, status, headers, body, durationMs } | { ok: false, error, detail }
 *
 * The allowlist is ported from the previous Vercel function (api/e-proxy.js), which was verified
 * against a real tenant. Deliberately standalone — this file must not import from src/.
 */
...
function hostAllowed(host) {
  ...
  if (DEFAULT_HOST_SUFFIXES.some((s) => host.endsWith(s))) return true;
  return extraHosts().includes(host); // exact match only — never wildcard user input
}
 ...
 */
export async function forward(envelope, res, { log = () => {}, warn = () => {}, doFetch = fetch } = {}) {
